...
async def warn_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_to_message.reply_text(f"⚠️ {user_to_warn.mention_html()} diberi peringatan oleh {admin_name}.\nTotal: <b>{total_warnings}/{warn_limit}</b>.")
    if total_warnings >= warn_limit:
        try:
            await context.bot.ban_chat_member(chat_id, user_to_warn.id)
            await update.effective_chat.send_message(f"🚫 {user_to_warn.mention_html()} dikeluarkan karena mencapai batas peringatan.")
            db_handler.clear_user_warnings(chat_id, user_to_warn.id)
        except Exception as e:
            await update.effective_chat.send_message(f"Gagal mengeluarkan {user_to_warn.mention_html()}.")
